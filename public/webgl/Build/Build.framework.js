...
var ASM_CONSTS = {
  4408000: function() {return Module.webglContextAttributes.premultipliedAlpha;},  
 4408061: function() {return Module.webglContextAttributes.preserveDrawingBuffer;},  
 4408125: function() {return Module.webglContextAttributes.powerPreference;},  
 4408183: function() {Module['emscripten_get_now_backup'] = performance.now;},  
 4408238: function($0) {performance.now = function() { return $0; };},  
 4408286: function($0) {performance.now = function() { return $0; };},  
 4408334: function() {performance.now = Module['emscripten_get_now_backup'];}
};
  ...
  function _GetAuthTokenFromJS() {
          try {
              ...
              // Try to get token from global window object
              ...
              console.log('AuthenticationManager: No authentication token found');
              ...
              console.error('AuthenticationManager: Error getting auth token:', e);
  ...
  function _GetUserIdFromJS() {
          try {
              ...
              // Try to get user ID from sessionStorage
              ...
              if (typeof window !== 'undefined' && window.userId) {
                  return parseInt(window.userId, 10) || 0;
              }
              ...
              console.log('AuthenticationManager: No user ID found');
              return 0;
          } catch (e) {
              console.error('AuthenticationManager: Error getting user ID:', e);
              return 0;
          }
      }
  ...
  function _RedirectToPage(pathPtr) {
          try {
              var path = UTF8ToString(pathPtr);
              console.log('Unity: Redirecting to ' + path);
              
              // Extract base path without query parameters
              var basePath = path.split('?')[0];
              var currentBasePath = window.location.pathname + window.location.hash;
              
              // Check if we're in an iframe
              if (window.parent !== window) {
                  // Send message to parent window to handle redirect
                  window.parent.postMessage({
                      type: 'unityRedirect',
                      path: path,
                      source: 'unity',
                      forceReload: basePath === '/account#chat'
                  }, '*');
                  
                  // Check if we're already on the chat page (ignoring query params)
                  if (currentBasePath === '/account#chat' || basePath === '/account#chat') {
                      // Force reload the parent window
                      window.parent.location.reload();
                  } else {
                      // Normal redirect for parent
                  ...
                  // Check if we're already on the chat page (ignoring query params)
                  if (currentBasePath === '/account#chat' || basePath === '/account#chat') {
                      // Force reload the current window
              ...
              console.error('Unity: Error redirecting to page:', e);
              // Fallback - try simple redirect
              window.location.href = UTF8ToString(pathPtr);
          }
      }

  function _RequestAuthTokenFromJS() {
          try {
              // Dispatch a custom event that the webpage can listen to
              if (typeof window !== 'undefined') {
                  // Try calling a global function if it exists
                  if (typeof window.requestAuthToken === 'function') {
                      window.requestAuthToken();
                  } else if (typeof window.getAuthToken === 'function') {
                      var token = window.getAuthToken();
                      if (token && typeof unityInstance !== 'undefined') {
                      ...
                      // Also try posting a message to parent window (for iframe scenarios)
                      if (window.parent !== window) {
                          window.parent.postMessage({
                  ...
                  console.log('AuthenticationManager: Requested authentication token from webpage');
              }
          } catch (e) {
              console.error('AuthenticationManager: Error requesting auth token:', e);
          }
      }
...
var asmLibraryArg = {
  "GetAuthTokenFromJS": _GetAuthTokenFromJS,
  "GetJSMemoryInfo": _GetJSMemoryInfo,
  "GetUserIdFromJS": _GetUserIdFromJS,
  ...
  "RedirectToPage": _RedirectToPage,
  "RequestAuthTokenFromJS": _RequestAuthTokenFromJS,
